...
console.log("Environment check:", {
  ...
  hasBrowserViewAPI:
    typeof window !== "undefined" && !!window.dockyard?.browserView,
});
...
if (typeof window !== "undefined" && !window.dockyard) {
  ...
  (window as any).dockyard = {
    ...
    workspaces: {
      ...
      delete: async (id: string) => {
        mockStorage.workspaces = mockStorage.workspaces.filter(
          (w) => w.id !== id,
        );
      },
      ...
      getActive: async () => {
        return (
          mockStorage.workspaces.find(
            (w) => w.id === mockStorage.activeWorkspaceId,
          ) || null
        );
      },
    },
  ...
  console.warn(
    "⚠️  Running in browser mode - BrowserView features will be mocked",
  );
}
