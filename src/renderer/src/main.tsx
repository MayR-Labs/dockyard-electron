...
if (typeof window !== 'undefined' && !window.dockyard) {
  ...
  (window as any).dockyard = {
    profiles: {
      list: async () => [],
      create: async (name: string) => ({ id: name, name, createdAt: new Date().toISOString(), lastAccessed: new Date().toISOString(), dataPath: '' }),
      delete: async () => {},
      switch: async () => {},
    ...
    workspaces: {
      ...
      create: async (data: any) => {
        const newWorkspace = {
          ...
          hibernation: data.hibernation || { enabled: true, idleTimeMinutes: 15 },
      ...
      update: async (id: string, data: any) => {
        ...
        if (idx !== -1) {
          mockStorage.workspaces[idx] = { ...mockStorage.workspaces[idx], ...data, updatedAt: new Date().toISOString() };
    ...
    apps: {
      ...
      update: async (id: string, data: any) => {
        ...
        if (idx !== -1) {
          mockStorage.apps[idx] = { ...mockStorage.apps[idx], ...data, updatedAt: new Date().toISOString() };
